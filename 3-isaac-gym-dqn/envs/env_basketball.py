...
class Basketball:
    ...
    def step(self, actions):
        ...
        '''
        이거 지우고 잘 만들기
        action tensor 출력해보면 (0,0) 한쌍으로 여러개 환경

        DQN 출력 예시)
        1번 action: 1번조인트 1, 2번조인트 0
        2번 action: 1번조인트 0, 2번 조인트 0
        ...

        '''

        # 2자유도의 행동 값을 각각 로봇 팔에 적용
        actions_tensor[::self.num_dof] = actions[:, 0]  # 첫 번째 관절
        actions_tensor[1::self.num_dof] = actions[:, 1]  # 두 번째 관절
...
def compute_arm_reward(ball_positions, ball_velocities, ball_radius, reset_buf, progress_buf, max_episode_length):
    ...
    '''
    보상함수 잘 설정하기
    리워드 보통 평균적으로 0~10 정도로 계산되게
    '''
    goal_position = torch.tensor([0.0, 3.0, 3.0], device="cuda")    # 목표 위치
    goal_threshold = 0.3
    ball_position = ball_positions

    distance_to_goal = torch.norm(ball_position - goal_position, dim=-1)    # 목표 위치에서 공의 거리

    # 공 거리가 goal_threshold 이내로 들어오면 100 보상 줘라
    reward = torch.where(distance_to_goal < goal_threshold, torch.tensor(100.0), torch.tensor(0.0))
    # 공 쳤는데 0.6보다 뒤로 날아가면 보상 -20
    ...
    # 공이 0.4보다 밑으로 가면 보상 -20 - 추가한 거
    reward += torch.where(ball_positions[..., 2] < 0.4, torch.ones_like(reward) * -20.0, reward)
    # 공이 0.4보다 밑으로 가면 리셋 - 추가한 거
    reset = torch.where(ball_positions[..., 2] < 0.4, torch.ones_like(reset_buf), reset)

    # 공이 6보다 위로 가면 보상 -20 - 추가한 거
    reward += torch.where(ball_positions[..., 2] > 6, torch.ones_like(reward) * -20.0, reward)
    # 공이 6보다 위로 가면 리셋 - 추가한 거
    reset = torch.where(ball_positions[..., 2] > 6, torch.ones_like(reset_buf), reset)

    # 공이 5보다 앞으로 가면 보상 -20 - 추가한 거
    reward += torch.where(ball_positions[..., 1] > 5, torch.ones_like(reward) * -20.0, reward)
    # 공이 5보다 앞으로 가면 리셋 - 추가한 거
    reset = torch.where(ball_positions[..., 1] > 5, torch.ones_like(reset_buf), reset)
    
    # 높이 보상 - 추가한거
    reward += torch.where((ball_positions[..., 1] <= 4) & (ball_positions[..., 1] > 3),
                          torch.ones_like(reward) * 10.0, reward)
    reward += torch.where((ball_positions[..., 1] <= 5) & (ball_positions[..., 1] > 4),
                          torch.ones_like(reward) * 20.0, reward)
    reward += torch.where((ball_positions[..., 1] <= 6) & (ball_positions[..., 1] > 5),
                          torch.ones_like(reward) * -10.0, reward)

    # 거리 보상 - 추가한거
    reward += torch.where((ball_positions[..., 1] <= 1.0) & (ball_positions[..., 1] > 0.7),
                          torch.ones_like(reward) * 10.0, reward)
    reward += torch.where((ball_positions[..., 1] <= 1.5) & (ball_positions[..., 1] > 1.0),
                          torch.ones_like(reward) * 20.0, reward)
    reward += torch.where((ball_positions[..., 1] <= 2.0) & (ball_positions[..., 1] > 1.5),
                          torch.ones_like(reward) * 30.0, reward)
    reward += torch.where((ball_positions[..., 1] <= 2.5) & (ball_positions[..., 1] > 2.0),
                          torch.ones_like(reward) * 40.0, reward)
    reward += torch.where((ball_positions[..., 1] <= 3.0) & (ball_positions[..., 1] > 2.5),
                          torch.ones_like(reward) * 50.0, reward)
    reward += torch.where((ball_positions[..., 1] <= 3.5) & (ball_positions[..., 1] > 3.0),
                          torch.ones_like(reward) * 60.0, reward)
